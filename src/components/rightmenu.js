    ...
    Vue.directive("rightMenu", {
      ...
      mounted(el, binding, vnode) {
        ...
        el.addEventListener("contextmenu", (e) => {
          ...
          // 右键显示遮罩层
          document.getElementById("TT_MASK").style =
            ...
            binding.value["menus"].map((item) => {
              ...
              if (item.callback) {
                optionp.onclick = function() {
                  ...
                  // 隐藏遮罩层
                  Mask.style = Maskstyle + "display:none";
                  return vnode.props[item.callback](item);
                };
              } else {
                // 无callback情况
                optionp.onclick = function() {
              ...
              let attrSelected = binding.value["selectOptions"]
                ? binding.value["selectOptions"]
                : {
                    background: "#eee",
                    color: "#000",
                  };
              let attrNoSelected = binding.value["noselectOptions"]
                ? binding.value["noselectOptions"]
                : {
                    background: "#fff",
                    color: "#000",
                  };
              optionp.onmouseenter = function() {
                this.style.background = attrSelected.background;
                this.style.color = attrSelected.color;
              };
              optionp.onmouseout = function() {
                this.style.background = attrNoSelected.background;
                this.style.color = attrNoSelected.color;
              };
              /**
               * 兼容屏幕出界的情况；
               */
              optionp.style = `padding:8px 10px;text-align:center;overflow: hidden;text-overflow:ellipsis;white-space:nowrap;margin-block-start: 0em;margin-block-end: 0em;-webkit-touch-callout: none;-webkit-user-select: none;-khtml-user-select: none;-moz-user-select: none;-ms-user-select: none;user-select: none;${
